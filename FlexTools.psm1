...
class XmlWatchdogConfiguration
{
    ...
    [bool] IsModulePresent([string]$moduleName)
    {
        ...
        $moduleNode = $this.ConfigXml.HERNISFlexWatchdogConfiguration.Modules.SelectSingleNode($moduleName)
        return $null -ne $moduleNode
    }

    [string] GetServerPort()
    {
        if (-not $this.IsLoaded) {
            return "4844" # Default port
        }
    ...
    {
        if (-not $this.IsLoaded) {
            return "C:\HERNIS\Logs" # Default log folder
        }
